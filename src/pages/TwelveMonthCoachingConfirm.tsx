...
export default function TwelveMonthCoachingConfirm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke(
        ...
        { body: { email, groupName: "PAID 12 Month Coaching" } }
      );
      if (fnError) throw fnError;
      if (data?.error) throw new Error(data.error);
      navigate("/12month-coaching-thankyou");
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <SEO
        title="Confirm Your Email — 12-Month Root Cause Partnership"
        description="Enter your email to receive your onboarding and scheduling link for the 12-Month Root Cause Partnership program."
        noindex
      />
      <Header />
      ...
      <main className="flex-grow flex items-center justify-center pt-20 md:pt-24 pb-16 md:pb-20">
        <div className="container-wellness">
          <div className="max-w-xl mx-auto text-center">
            <p className="section-label mb-4">Deepest Transformation Partnership Package</p>
